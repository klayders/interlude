...
public class FishingChampionShipManager {
  ...
  private void restoreData() {
    ...
    try {
      ...
      log.error("Exception: can't get fishing championship info: " + var8.getMessage());
    } finally {
  ...
  public String getWinnerName(int par) {
    return this._winPlayersName.size() >= par ? this._winPlayersName.get(par - 1) : "—";
  }

  public String getCurrentName(int par) {
    return this._playersName.size() >= par ? this._playersName.get(par - 1) : "—";
  }
  ...
  public void getReward(Player pl) {
    ...
    while (true) {
      ...
      for (int x = 0; x < this._winPlayersName.size(); ++x) {
        if (this._winPlayersName.get(x).equalsIgnoreCase(pl.getName())) {
          switch (x) {
            case 0:
  ...
  public void shutdown() {
    ...
    try {
      ...
      log.error("Exception: can't update player vitality: " + var8.getMessage());
    } finally {
      DbUtils.closeQuietly(con, statement);
    }
  ...
  private synchronized void refreshResult() {
    ...
    FishingChampionShipManager.Fisher fisher1;
    FishingChampionShipManager.Fisher fisher2;

    int x;
    for (x = 0; x <= this._tmpPlayers.size() - 1; ++x) {
      for (int y = 0; y <= this._tmpPlayers.size() - 2; ++y) {
        fisher1 = this._tmpPlayers.get(y);
        fisher2 = this._tmpPlayers.get(y + 1);
    ...
    for (x = 0; x <= this._tmpPlayers.size() - 1; ++x) {
      this._playersName.add(this._tmpPlayers.get(x)._name);
  ...
  private void refreshWinResult() {
    ...
    for (x = 0; x <= this._winPlayers.size() - 1; ++x) {
      for (int y = 0; y <= this._winPlayers.size() - 2; ++y) {
        fisher1 = this._winPlayers.get(y);
        fisher2 = this._winPlayers.get(y + 1);
        if (fisher1.getLength() < fisher2.getLength()) {
    ...
    for (x = 0; x <= this._winPlayers.size() - 1; ++x) {
      this._winPlayersName.add(this._winPlayers.get(x)._name);
  ...
  private class finishChamp extends RunnableImpl {
    ...
    public void runImpl() throws Exception {
      ...
      for (Fisher fisher : FishingChampionShipManager.this._tmpPlayers) {

...
public abstract class L2GameClientPacket extends ReceivablePacket<GameClient> {
  ...
  public final boolean read() {
    try {
      ...
      log.error("Client: " + this._client + " - Failed reading: " + this.getType() + " - Server Version: " + GameServerConfiguration.getInstance().getVersion().getRevisionNumber(), var2);
    } catch (Exception var3) {
      log.error("Client: " + this._client + " - Failed reading: " + this.getType() + " - Server Version: " + GameServerConfiguration.getInstance().getVersion().getRevisionNumber(), var3);
    }
  ...
  public final void run() {
    ...
    try {
      ...
      log.error("Client: " + client + " - Failed running: " + this.getType() + " - Server Version: " + GameServerConfiguration.getInstance().getVersion().getRevisionNumber(), var3);
    }

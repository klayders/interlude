...
public class RequestActionUse extends L2GameClientPacket {
    ...
    if (activeChar != null) {
      ...
      if (action == null) {
        ...
        if (!usePet && (activeChar.isOutOfControl() || activeChar.isActionsDisabled()) && (!activeChar.isFakeDeath() || this._actionId != 0)) {
          activeChar.sendActionFailed();
        } else if ((activeChar.getTransformation() != 0 || activeChar.isCursedWeaponEquipped()) && action.transform > 0) {
          activeChar.sendActionFailed();
          ...
          if (!activeChar.isOutOfControl() && activeChar.getTransformation() == 0 && !activeChar.isActionsDisabled() && !activeChar.isSitting() && activeChar.getPrivateStoreType() == 0 && !activeChar.isProcessingRequest()) {
            if (activeChar.isFishing()) {
              ...
              if (Config.ALT_SOCIAL_ACTION_REUSE) {
                ThreadPoolManager.getInstance().schedule(new RequestActionUse.SocialTask(activeChar), 2600L);
          ...
          if (action.type == 2) {
            ...
            switch(action.id) {
              ...
              case 22:
                ...
                if (!this._ctrlPressed && target.isCreature() && !((Creature)target).isAutoAttackable(pet)) {
                  return;
                }
              ...
              case 37:
                ...
                break;
              case 38:
                if (activeChar.getTransformation() == 0 && !activeChar.isCursedWeaponEquipped()) {
  ...
  private void UseSkill(int skillId) {
    ...
    if (pet == null) {
      ...
      if (skillLevel == 0) {
        ...
        if (skill == null) {
          activeChar.sendActionFailed();
        } else if (activeChar.getLevel() + 20 <= pet.getLevel()) {
          activeChar.sendPacket(SystemMsg.YOUR_PET_IS_TOO_HIGH_LEVEL_TO_CONTROL);
        } else {
  ...
  public static enum Action {
    ...
    public static final RequestActionUse.Action[] VALUES = values();

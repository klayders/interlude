...
public final class WarehouseFunctions {
  ...
  public static boolean canShowWarehouseWithdrawList(Player player, WarehouseType type) {
    if (!player.getPlayerAccess().UseWarehouse) {
      ...
      if (((Warehouse)warehouse).getSize() == 0) {
        player.sendPacket((IStaticPacket)(type == WarehouseType.FREIGHT ? SystemMsg.NO_PACKAGES_HAVE_ARRIVED : Msg.YOU_HAVE_NOT_DEPOSITED_ANY_ITEMS_IN_YOUR_WAREHOUSE));
        return false;
      } else {
        return true;
      }
